...
func SubByte(inbyte byte) (outbyte byte) {
	var x, y byte
	x = inbyte >> 4
	y = inbyte & 0x0f
	return sBox[x][y]
...
func SubCol(incol []byte) (outcol []byte) {
	outcol = make([]byte, len(incol))
	for i := 0; i< len(incol); i++ {
		outcol[i] = SubByte(incol[i])
	}
	return outcol
}
...
func InvSubByte(inbyte byte) (outbyte byte) {
	var x, y byte
	x = inbyte >> 4
	y = inbyte & 0x0f
	return invsBox[x][y]
...
func InvSubCol(incol []byte) (outcol []byte) {
	outcol = make([]byte, len(incol))
	for i := 0; i< len(incol); i++ {
		outcol[i] = InvSubByte(incol[i])
	}
	return outcol
}

func InvSubState(instate State) (outstate State) {
	outstate = make([][]byte, 4)
	for i := 0; i < len(instate); i++ {
		outstate[i] = InvSubCol(instate[i])
	}
	return outstate
}
